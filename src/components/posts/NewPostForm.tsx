...
import { createNews, getNews, updateNews } from "@/lib/api/news";
...
export function NewPostForm({ mode = "create", editId }: NewPostFormProps) {
  ...
  useEffect(() => {
    ...
    const load =
      initialType === "News"
        ? getNews(editId).then((result) => result.post)
        : getProject(editId).then((result) => result.project);
    ...
    load.then((item) => {
      ...
      setCoverImageUrl(item.thumbnailUrl ?? null);
      setBody(item.body ?? "");
      if ("category" in item) {
        setCategory(item.category);
        setOriginalExcerpt(item.excerpt);
      } else {
        setCategory(item.focusArea);
        setOriginalExcerpt(item.summary);
      }
      setLoadingExisting(false);
    });
  ...
  async function handleSubmit(status: "Published" | "Draft") {
    if (!title.trim() || (mode === "create" && !body.trim())) {
    ...
    const tags = tagsInput
      ...
      .map((tag) => tag.trim())
      .filter(Boolean);
    const excerpt = body.trim() ? deriveExcerpt(body) : originalExcerpt;

    if (mode === "edit" && editId) {
      if (postType === "News") {
        await updateNews(editId, { title, excerpt, category, tags, thumbnailUrl: coverImageUrl, body, status });
        router.push("/news");
      } else {
    ...
    if (postType === "News") {
      await createNews({ title, excerpt, category, tags, thumbnailUrl: coverImageUrl, body, status });
      router.push("/news");
    } else {
      await createProject({
        title,
  ...
  const excerptPreview = body.trim() ? deriveExcerpt(body) : originalExcerpt;

  if (mode === "edit" && notFound) {
    return (
      <div className="mt-3 flex h-full min-h-0 flex-col items-center justify-center text-center">
        <p className="text-[15px] text-neutral-500">
          That {initialType === "News" ? "post" : "project"} couldn&apos;t be found.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex mt-3 h-full min-h-0 flex-col overflow-hidden">
      <div className="shrink-0">
        <h1 className="text-[22px] font-semibold text-neutral-900">{mode === "edit" ? "Edit Post" : "New Post"}</h1>
      ...
      {loadingExisting ? (
        <div className="mt-5 flex flex-1 items-center justify-center text-[13px] text-neutral-400">Loading...</div>
      ) : (
            ...
            <label className="block">
              ...
              <div className="grid grid-cols-2 gap-1 rounded-[10px] bg-neutral-100 p-1">
                <button
                  ...
                  disabled={mode === "edit"}
                  className={cn(
                    "h-9 rounded-[10px] text-[12px] font-medium transition-colors disabled:cursor-not-allowed",
              ...
              >
                {coverImageUrl ? (
                  <span className="flex items-center gap-2 text-[12px] text-neutral-700">
                    ...
                    {coverImageName || "Current image — click to replace"}
                  </span>
                ) : (
                  <>

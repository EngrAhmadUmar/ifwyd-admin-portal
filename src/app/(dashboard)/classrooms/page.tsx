...
const COLUMNS = ["Name", "Subject", "Schedule", "Enrolled", "Status", "Action"] as const;
...
export default function ClassroomsPage() {
  ...
  const load = useCallback(async (pageNum: number) => {
    setLoading(true);
    const result = await fetchClassrooms(pageNum, CLASSROOMS_PER_PAGE);
  ...
  const totalPages = Math.max(1, Math.ceil(total / CLASSROOMS_PER_PAGE));

  async function handleConfirm() {
    if (!pendingAction) return;
    const { type, id } = pendingAction;
    setActionError(null);
    const nextStatus: ClassroomStatus = type === "activate" ? "Active" : "Closed";
    ...
    if (source === "api") {
      try {
        if (type === "delete") {
          ...
          await updateClassroomStatus(id, nextStatus);
        }
        await load(page);
      } catch (error) {
  ...
  return (
    <div className="flex mt-3 h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-start justify-between gap-3">
        <div>
          <h1 className="text-[22px] font-[600] text-black">Classrooms</h1>
      ...
      <div className="mt-5 flex min-h-0 flex-1 flex-col rounded-[10px] border border-neutral-200/80 bg-white shadow-sm">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {loading ? (
            <p className="py-12 text-center text-[13px] text-neutral-400">Loading classrooms...</p>
          ) : items.length === 0 ? (
            <p className="py-12 text-center text-[13px] text-neutral-400">No classrooms yet.</p>
          ) : (
            items.map((classroom) => (
              <div key={classroom.id} className={cn("grid items-center border-b border-neutral-100 px-6 py-3.5 last:border-0 hover:bg-neutral-50/60", COL_GRID)}>
                <span className="truncate pr-4 text-[13px] font-[500] text-black">{classroom.name}</span>
                <span className="text-[13px] font-[400] text-black">{classroom.subject}</span>
                <span className="text-[13px] font-[400] text-black">{classroom.schedule}</span>
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-neutral-200/80 px-6 py-3.5">
          <span className="text-[13px] font-[400] text-black">
            Showing {items.length} of {total.toLocaleString()} classrooms
          </span>
